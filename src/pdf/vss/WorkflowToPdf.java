...
public class WorkflowToPdf extends BaseCronJob {
	...
	public void doservice() {
		...
		sql = "select * from workflow_requestbase  where requestid>76192 and currentnodetype=3";
		rs.execute(sql);
		while(rs.next()) {
			pdfUrl = createPdf(Util.null2String(rs.getString("requestid")),oaaddress);
			log.writeLog("pdfUrl:"+pdfUrl);
		}
	}
	
	public String createPdf(String requestid,String oaaddress) {
		BaseBean log = new BaseBean();
		RecordSet rs = new RecordSet();
		String workflowid = "";
		String requestname = "";
		...
		String exepathString = "D:\\wkhtmltopdf\\bin\\wkhtmltopdf.exe";
		...
		String infopathString = "TableToPdf" + File.separator + requestid+ File.separator + ramid
				+ File.separator;
		...
		log.writeLog("html 11:" + loadPathString + requestid + ".html");
		log.writeLog("pdf 11:" + savePathString + requestid + ".pdf");
		String[] args = { exepathString, loadPathString + requestid + ".html", savePathString + requestid + ".pdf" };
		...
		return oaaddress+"/TableToPdf/"+requestid+"/"+ramid+"/"+requestid+".pdf";
	}
